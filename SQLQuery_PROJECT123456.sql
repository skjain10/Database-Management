...
(1, 'John','F',25,'M','Trainer'),
(2, 'Kim','D',24,'M','Trainer'),
(3, 'Sam','J',27,'M','Receptionist'),
(4, 'Emily','S',28,'F','Receptionist'),
(5,'Hannah','B',24,'F','Trainer'),
(6,'Tim','J',28,'M','Trainer'),
(7, 'Rahul','R',22,'M','Trainer'),
(8, 'Ritika','S',23,'F','Trainer'),
(9, 'Collins','D',35,'M','Receptionist'),
(10, 'Kanisha','P',22,'F','Cleaner'),
(11,'Kirk','D',27,'M','Cleaner'),
(12,'Jolton','A',27,'M','Cleaner'),
(13,'Merick','G',34,'M','Trainer'),
(14,'Austin','Q',34,'M','Trainer'),
...
Select mem.mID,mFName,mLName,count(mem.mID) 'No.of Products Ordered'
from Member mem
Join order1 O
ON mem.mID = O.mID
Join orderLine1 L
on O.orderIDID = L.orderIDID
group by mem.mID,mFName,mLName
...
Group by T.trainerID,eFName
 
--2)
Select P.pID,pName,oName,count(P.pID) 'No. Of Members enrolled'
from Member mem
JOIN Membership M
ON M.mID = mem.mID
Join Plan1 P
...
Join Offer O
ON O.oID=M.oID
where OName='BlackFriday' and pName='Regular'
Group by P.pID,pName,oName

--4)
Select mStreetName,count(mStreetName)'No.of Members'
from Member
Group by mStreetName

--3)
Create View View_MP as 
Select mem.mID,mFName,mLName,P.pID,pName
from Member mem
JOIN Membership M
ON M.mID = mem.mID
Join Plan1 P
ON P.pID = M.pID 
Group by mem.mID,mFname,mLName,P.pID,P.pName
...
Create View View_ProdMax1 as
select OL.UPC,productName,productType,count(OL.UPC) 'No_of_Orders'
from order1 O
Join orderline1 OL
ON O.orderIDID = OL.orderIDID
Join Product1 P
on OL.UPC= P.UPC
GROUP BY OL.UPC,productName,productType

SELECT * 
FROM View_ProdMax1 
WHERE  No_Of_Orders = (SELECT MAX(No_of_Orders) FROM View_ProdMax1);
...
Insert into Member 
Values (23,'Ramesh','Suresh','M',37,3157875676,299,'Waverly',2)
Insert into Membership
Values(23,3,'2018-11-20',4,'2019-02-20',1)
